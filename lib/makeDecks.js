...
function makeCard(card, templates, deckDiv) {
    ...
    if (template.portrait) {
        $('<img class="portrait", src="images/portraits/' + card.fields.name.replace(/[^0-9a-z]/gi, '').toLowerCase() + '.png" onerror="this.style.visibility = \'hidden\'">').appendTo(cardDiv);
    }
    ...
    if (template.faders) {
        for (var i = 0; i < template.faders.length; i++) {
            ...
            for (var j = 0; j < fader.tops.length; j++) {
                ...
                var opacity = 1.;
                var width = fader.width;
                if (j >= value) {
                    var opacity = 0.4;
                }
                $('<img class="icon", src="images/' + fader.image + '", style="width:' + width + '; top:' + tops[j] + 'em; left:' + lefts[j] + 'em;opacity:' + opacity + ';">').appendTo(cardDiv);
            }
        }
    ...
    if (template.options) {
        for (var i = 0; i < template.options.length; i++) {
            var option = template.options[i]
            var image = option[card.fields.data[option.index]];
            var top = option.top;
            var left = option.left;
            var width = option.width;
            console.log("image")
            console.log(image)
            $('<img class="icon", src="images/' + image + '", style="width:' + width + '; top:' + top + '; left:' + left + ';">').appendTo(cardDiv);
        }
    }
